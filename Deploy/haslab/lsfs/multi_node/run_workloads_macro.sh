...
for CONFIG_P in ${PEERS_CONFIG[@]}; do
    ...
    for ((RUN_ITER=1; RUN_ITER<=NR_OF_ITERATIONS_PER_WORKLOAD; RUN_ITER++)); do
        ...
        for LB_TYPE in ${WORKLOAD_VAR_LB_TYPE[@]}; do
                ...
                ansible-playbook deploy/change_run_config.yml -e "remote_com_directory=$REMOTE_COM_DIRECTORY load_balancer=$LB_TYPE nr_puts_req=$NR_PUTS_REQUIRED nr_gets_req=$NR_GETS_REQUIRED nr_gets_vrs_req=$NR_GETS_VERSION_REQUIRED use_cache=$USE_CACHE cache_refresh=$CACHE_REFRESH paralelization_w=$WL_CONF_PARAL_LIMIT_WRITE paralelization_r=$WL_CONF_PARAL_LIMIT_READ wl_conf_io=$WL_CONF_IO wl_path=$wl_remote_path" -i deploy/hosts -v
        ...
        for ENTROPY_INTERVAL in ${WORKLOAD_VAR_ANTI_ENTROPY_INTERVAL[@]}; do
                ...
                ansible-playbook deploy/change_run_config.yml -e "remote_com_directory=$REMOTE_COM_DIRECTORY load_balancer=$LB_TYPE nr_puts_req=$NR_PUTS_REQUIRED nr_gets_req=$NR_GETS_REQUIRED nr_gets_vrs_req=$NR_GETS_VERSION_REQUIRED use_cache=$USE_CACHE cache_refresh=$CACHE_REFRESH entropy_interval=$ENTROPY_INTERVAL paralelization_w=$WL_CONF_PARAL_LIMIT_WRITE paralelization_r=$WL_CONF_PARAL_LIMIT_READ wl_conf_io=$WL_CONF_IO wl_path=$wl_remote_path" -i deploy/hosts -v
