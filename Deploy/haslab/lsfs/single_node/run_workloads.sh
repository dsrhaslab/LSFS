...
#WORKLOAD_VAR_IO_SIZE=("32k" "64k" "96k")
...
ansible-playbook deploy/1_setup_deploy.yml -e "remote_com_directory=$REMOTE_COM_DIRECTORY image_name=$IMAGE_NAME" -i $ANSIBLE_HOSTS_PATH -v
...
# Run metadata workloads
    ...
    #             ansible-playbook deploy/change_run_config.yml -e "remote_com_directory=$REMOTE_COM_DIRECTORY load_balancer=$LOAD_BALANCER use_cache=$USE_CACHE cache_refresh=$REFRESH_CONF paralelization=$WL_CONF_PARAL_LIMIT wl_conf_io=$WL_CONF_IO wl_path=$wl_remote_path" -i $ANSIBLE_HOSTS_PATH -v
